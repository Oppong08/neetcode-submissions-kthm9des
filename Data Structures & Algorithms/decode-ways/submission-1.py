class Solution:
    def numDecodings(self, s: str) -> int:
        ...
        #create a dp cache to store number of ways for decoding the current number of strings in consideration 
        ...
        def dfs(i):
            ...
            #subproblem 1: taking the number as one digit ( 0 - 9)
            res = dfs(i + 1)

            #subproblem 2 : taking the number as a double digit and
            #first #(starts with 1, ie 0 - 9)
            if i + 1 < len(s) and (s[i] == "1" or 
                s[i] == "2" and s[i +1] in "01234567"):#(starts with 2 but ends at 26, ie 20-26)
                res += dfs(i + 2)
            dp[i] = res
            return res
